...
server {
    ...
    # Documentación de la API -> Spring Boot (springdoc)
    #   /swagger-ui.html, /swagger-ui/index.html, /v3/api-docs, /v3/api-docs.yaml
    # "^~" evita que la regla de archivos estáticos (.js/.css/.png) intercepte los recursos de Swagger UI.
    # $http_host conserva el puerto (ej. localhost:8081) para que las redirecciones de Swagger funcionen.
    location ^~ /swagger-ui {
        proxy_pass         http://127.0.0.1:8080;
        proxy_http_version 1.1;
